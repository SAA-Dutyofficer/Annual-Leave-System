...
async function loadTeam() {
  ...
  try {
    ...
    el.innerHTML=members.map(m=>{
      const onLeave=allRequests.some(r=>r.employeeId===m.id&&r.status==="Approved"&&r.startDate<=today2&&r.endDate>=today2);
  ...
  } catch{el.innerHTML=`<div class="list-empty">Could not load team.</div>`;}
}
